...
*/
func calcArea(x: Double, y: Double) -> Double? {
    guard x > 0 && y > 0 else {
        return nil
    }
    return x * y;
}
print(calcArea(x: 0.1, y: 217.89)!)
print(calcArea(x: -0.1, y: 217.89))
 ...
 */
func add(fir: Int?, sec: Int?) -> Int? {
    guard let f = fir,
        let s = sec else { return nil }
    return f + s;
}
print(add(fir: nil, sec: 17))
print(add(fir: 2, sec: 17)!)
